...
for task in args.task_name.split(','):
    for fileName in ['train', 'dev', 'test']:
        ...
        for tmp_idx in range(args.best):
            data_emoji_all.append([copy.deepcopy(train_dataset),copy.deepcopy(train_dataset)])
            ...
            with torch.no_grad():
                ...
                for sp in range(args.split):
                    ...
                    for tmp_idx in best_idx.cpu().numpy():
                        ...
                        best_emoji.append(emoji_tags[sp][int(tmp_idx/args.num_samples)])
                    del dis
                ...
                for tmp_idx in range(args.best):
                    ...
                    for cur_idx in best_idx:
                        ...
                        data_emoji_all[tmp_idx][1][idx]['text'] = ' '+data_emoji_all[tmp_idx][1][idx]['text'].strip() + ' \n '\
                                                                + tokenizer.decode(best_text[cur_idx][1:]).replace(tokenizer.pad_token,'').strip() \
                                                                +  ' ' + best_emoji[cur_idx] + ' \n '

        for tmp_idx in range(args.best):
            write_json(data_emoji_all[tmp_idx][0],
                       args.dataset_path + task + '/' + fileName + args.method + '_top' + str(tmp_idx) \
                       + '_' + 'textfirst')
            write_json(data_emoji_all[tmp_idx][1],
                       args.dataset_path + task + '/' + fileName + args.method + '_top' + str(tmp_idx) \
                       + '_' + 'textlast')
