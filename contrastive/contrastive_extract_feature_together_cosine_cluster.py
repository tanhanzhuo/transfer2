...
parser.add_argument('--file',default='selected_test',type=str)
...
for step, batch in enumerate(train_data_loader):
    with torch.no_grad():
        ...
        if labels[0] != labels[-1] or labels[0] != previous_label or step == MAX_LEN:#goes to another hashtag
            ...
            kmeans = KMeans(n_clusters=args.num_sample).fit(preprocessing.normalize(embeddings))
            for center in kmeans.cluster_centers_:
                center_embs.append(center)
            ...
            print('current hashtag:{}, {}, number hashtag:{}, cur hash sample:{}, total hash samples:{}'. \
                  format(previous_label.item(),CONVERT[str(previous_label.item())], total_num, len(embeddings), len(center_embs)))
