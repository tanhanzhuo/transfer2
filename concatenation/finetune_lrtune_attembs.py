...
class RobertaClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.att = ScaledDotProductAttention(d_model=config.hidden_size, d_k=config.hidden_size, d_v=config.hidden_size, h=8)

    def forward(self, features, embs, **kwargs):
        x = features[:, 0, :]  # take <s> token (equiv. to [CLS])
        x = self.att(x.unsqueeze(1), embs, embs).squeeze(1) ################################ attentions w.r.t. 10 samples
        x = self.dropout(x)
        x = self.dense(x)
        x = torch.tanh(x)
        x = self.dropout(x)
...
class RobertaForMulti(RobertaPreTrainedModel):
    ...
    def forward(self,
        ...
        logits = self.classifier(sequence_output, embs)
        return logits
...
def parse_args():
    ...
    parser.add_argument(
        "--method",
        default='feature_modelT100N100S_fileT100S_num10',
        type=str,
        required=False,
        help="The output directory where the model predictions and checkpoints will be written.",
    )
...
def evaluate(model, data_loader):
    ...
    for batch in data_loader:
        input_ids, segment_ids, labels, embs = batch
        logits = model(input_ids.cuda(), embs.cuda(), segment_ids.cuda())
