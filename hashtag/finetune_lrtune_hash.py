...
class RobertaForMulti(RobertaPreTrainedModel):
    ...
    def __init__(self, config, config2):
        ...
        self.classifier_hash = RobertaClassificationHead(config2)
    ...
    def forward(self,
        ...
        logits_hash = self.classifier_hash(sequence_output)
        return logits, logits_hash
...
def parse_args():
    ...
    parser.add_argument(
        "--task_name",
        default='stance/face_masks_sp,stance/fauci_sp,stance/school_closures_sp,stance/stay_at_home_orders_sp,hate,sem-17,sem-18,wtwt/CVS_AET,wtwt/CI_ESRX,wtwt/ANTM_CI,wtwt/AET_HUM,wtwt/FOXA_DIS',
        type=str,
        required=False,
    ...
    parser.add_argument(
        "--num", default='100', type=str, help="top hashtag class number")
    parser.add_argument(
...
def do_train(args):
    ...
    for lr in learning_rate:
        ...
        config2 = AutoConfig.from_pretrained(args.model_name_or_path, num_labels=int(args.num))
        ...
        loss_fct_hash = nn.CrossEntropyLoss().cuda()
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_data_loader):
                global_step += 1
                input_ids, segment_ids, labels, labels_hash = batch
                logits, logits_hash = model(input_ids, segment_ids)
                loss = loss_fct(logits, labels.view(-1))
                loss_hash = loss_fct_hash(logits_hash, labels_hash.view(-1))
                loss_all = loss * int(args.ratio) / 10.0 + loss_hash * (10 - int(args.ratio)) / 10.0
                # print(step)
            ...
            for ratio in args.ratio.split(','):#range(10, 0, -2):
                for ratio2 in args.ratio2.split(','):#range(10, -2, -2):
                    ...
                    for seed in [1, 10, 100, 1000, 10000]:
                        ...
                        args_tmp.input_dir = args.input_dir + task + '/hash_' + str(args.num)
