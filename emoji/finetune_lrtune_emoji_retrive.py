...
class RobertaForMulti(RobertaPreTrainedModel):
    ...
    def __init__(self, config, config2):
        ...
        self.classifier_class = RobertaClassificationHead(config2)
...
def evaluate(model, data_loader):
    ...
    for batch in data_loader:
        ...
        logits, logits_class = model(input_ids, segment_ids)
...
def do_train(args):
    ...
    for lr in learning_rate:
        ...
        config2 = AutoConfig.from_pretrained(args.model_name_or_path, num_labels=num_classes)
        ...
        loss_fct_class = nn.CrossEntropyLoss().cuda()
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_data_loader):
                ...
                logits, logits_class = model(input_ids, segment_ids)
                loss = loss_fct(logits, labels_emoji.view(-1))
                loss_class = loss_fct_class(logits_class, labels.view(-1))
                loss_all = loss * (10 - int(args.ratio)) / 10.0 + loss_class * int(args.ratio) / 10.0
                # print(step)
