...
for emoji_one in emoji_top:
    emoji_token.append([tokenizer._convert_token_to_id(emoji.demojize(emoji_one))])

emoji_str_token = []
for emoji_one in emoji_top:
    emoji_str = ' '.join( emoji.demojize(emoji_one)[1:-1].split('_') )
    emoji_str = tokenizer(emoji_str)['input_ids'][1:-1]
    emoji_str_token.append(emoji_str)
...
def parse_args():
    ...
    parser.add_argument(
        "--task_name",
        default='stance,hate,sem-18,sem-17,wtwt',
        type=str,
        required=False,
    ...
    parser.add_argument(
        "--emoji_or_str", default='emoji', type=str, help="ratio for loss")
...
def convert_example(example, label2idx, flag):
    example.pop('attention_mask')
    if flag == 'emoji':
        emoji_one = emoji_token[example['emoji']]
    else:
        emoji_one = emoji_str_token[example['emoji']]
    for one in emoji_one:
        example['input_ids'].insert(-1, one)
        example['token_type_ids'].append(tokenizer.pad_token_type_id)
