...
def do_train(args):
    ...
    weight = np.array([0.21079396, 0.07110703, 0.13296211, 0.11333127, 0.04671113, 0.4250945])
    weight = sum(weight)/weight
    weight = weight/sum(weight)
    weight = torch.FloatTensor(weight).cuda()
    loss_fct = torch.nn.CrossEntropyLoss(weight=weight).cuda()
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_data_loader):
            ...
            output = model(**batch)
            loss = loss_fct(output.logits,labels.view(-1))
