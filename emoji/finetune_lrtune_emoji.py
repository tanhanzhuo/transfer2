...
def parse_args():
    ...
    parser.add_argument(
        "--task_name",
        default='stance/face_masks_new,stance/fauci_new,stance/school_closures_new,stance/stay_at_home_orders_new,hate,sem-17,sem-18,wtwt/CVS_AET,wtwt/CI_ESRX,wtwt/ANTM_CI,wtwt/AET_HUM,wtwt/FOXA_DIS',
        type=str,
        required=False,
...
def evaluate(model, data_loader):
    ...
    acc = rep['accuracy']
    logging.info("aveRec:%.5f, f1PN:%.5f, acc: %.5f " % (f1, acc, acc))
    return f1, acc, acc
...
def evaluate_17(model, data_loader):
    ...
    acc = rep['accuracy']
    logging.info("aveRec:%.5f, f1PN:%.5f, acc: %.5f " % (aveRec, f1PN, acc))
...
def evaluate_18(model, data_loader):
    ...
    acc = rep['accuracy']
    logging.info("f1_pos:%.5f, f1PN:%.5f, acc: %.5f " % (f1_pos, f1PN, acc))
...
def do_train(args):
    ...
    for lr in learning_rate:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if (epoch + 1) % args.save_steps == 0:
                ...
                logging.info("eval done total : %s s" % (time.time() - tic_eval))
    ...
    logging.info('final')
    logging.info("f1macro:%.5f, acc:%.5f, acc: %.5f, " % (best_metric[0], best_metric[1], best_metric[2]))
    logging.info("f1macro:%.5f, acc:%.5f, acc: %.5f " % (cur_metric[0], cur_metric[1], cur_metric[2]))
    del model
    return cur_metric
        ...
        for model_name in args.model_name_or_path.split(','):  # [r_dir+'bertweet/']:
            for ratio in args.ratio.split(','):#range(10, 0, -2):
                for ratio2 in args.ratio2.split(','):#range(10, -2, -2):
                    ...
                    logging.info("*************************************************************************************")
                    logging.info('Task: %s, model: %s, loss ration: %s, weight ratio: %s, seed: %d' % (task, model_name, ratio, ratio2, seed))
                    logging.info('final aveRec:%.5f, f1PN:%.5f, acc: %.5f ' % (sum(ave_metric[:, 0]) / 5,
                                                                        sum(ave_metric[:, 1]) / 5,
                                                                        sum(ave_metric[:, 2]) / 5))
