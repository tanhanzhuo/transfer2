...
import numpy as np
import argparse
from transformers import AutoTokenizer, AutoConfig, AutoModel,DataCollatorWithPadding,set_seed, RobertaForMaskedLM, RobertaForSequenceClassification
import os

parser = argparse.ArgumentParser()
parser.add_argument('--bt',default='./bertweet/',type=str)
parser.add_argument('--ori',default='./10/',type=str)
parser.add_argument('--save',default='./10_new/',type=str)
...
parser.add_argument('--do_mlm',default=True,type=bool)
...
for k, v in model_ori.items():
    # k=k.replace('roberta.','').replace("mlp","pooler")
    k = k.replace("mlp", "pooler")
    model_new[k]=v
# model3 = OrderedDict(('pooler' if k == 'mlp' else k, v) for k, v in model2.items())
if not os.path.isdir(args.save[:-1]+'_mlm/'):
    os.mkdir(args.save[:-1]+'_mlm/')
torch.save(model_new,args.save[:-1]+'_mlm/'+'pytorch_model.bin')
import os
import shutil
shutil.copyfile(args.bt+'/config.json',args.save[:-1]+'_mlm/' + "/config.json")
...
# model_new = RobertaForSequenceClassification.from_pretrained(args.save)
model_new.eval()
