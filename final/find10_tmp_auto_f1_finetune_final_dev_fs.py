...
def fewshot(args):
    ...
    with open(args.input_dir + task + '/train' + method + '.json', 'r', encoding='utf-8') as f:
        for line in f:
            one = json.loads(line)
            data.append(one)
    ...
    select_idx = []
    select_idx_dic = {}
    for val in CONVERT[args.task].keys():
        select_idx_dic[val] = 0
    for idx in range(len(data)):
        label_tmp = data[idx]['labels']
        if select_idx_dic[label_tmp] < sample_num:
            select_idx.append(idx)
            select_idx_dic[label_tmp] += 1
    np.random.shuffle(select_idx)
    ...
    with open(args.input_dir + task + '/train' + method + '_'+ args.shot +'.tsv', 'w', encoding='utf-8') as f:
...
def tokenization(args):
    ...
    data_files_hash = {}
    method = args.method.split('hash')[1].split('top')[0] + 'top100_sp' + args.rerank
    data_files_hash["train"] = args.input_dir + task + '/train' + method + '_'+ args.shot +'.json'
    data_files_hash["dev"] = args.input_dir + task + '/dev' + method +'.json'
            ...
            for model_name in args.model_name_or_path.split(','):  # [r_dir+'bertweet/']:
                ...
                for seed in args.seed.split(','):
                    ...
                    if shot != 'full':
                        ...
                        args_tmp.train = args.input_dir + task + '/' + \
                                         'train' + args.method.split('hash')[1].split('top')[0] + 'top100_sp' + args.rerank+ '_' + shot + '.tsv'
                    # args_tmp.train = Path(args_tmp.train)
