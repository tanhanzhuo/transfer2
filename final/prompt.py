...
def parse_args():
    ...
    parser.add_argument(
        "--method",
        default='_seg_500_one20_top100_sp',
        type=str,
        required=False,
        help="The output directory where the model predictions and checkpoints will be written.",
    )
...
def do_train(args):
    ...
    for split in SPLITS:
        ...
        for data in data_all:
            if args.demo > 0:
                text_demo = data['text0']
                for idx in range(1, args.demo):
                    text_demo = data['text'+str(idx)] + ' </s> ' + text_demo
                input_example = InputExample(text_a=data['text'], text_b=text_demo, label=int(label2idx[data['labels']]))
            else:
    ...
    for lr in learning_rate:
        ...
        if args.template is None:
            if args.demo == 0:
                ...
                template_text = '{"placeholder":"text_b"} </s> {"placeholder":"text_a"}' + TEMPLATE[args.choice][args.task]
        else:
        ...
        if args.soft == 0:
            ...
            myverbalizer = SoftVerbalizer(tokenizer, plm, num_classes=len(label2idx.keys()), label_words=label_words)
        elif args.soft == 3:
            label_words = []
