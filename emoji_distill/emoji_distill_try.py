...
class RobertaForTeacher(RobertaPreTrainedModel):
...
def teacher_loss(logits_pred, logits_teacher, T):
    pred_prob = nn.functional.log_softmax(logits_pred/T, dim=-1)
    teacher_prob = nn.functional.softmax(logits_teacher/T, dim=-1)
    loss = nn.functional.kl_div(pred_prob, teacher_prob, reduction='sum') * (T**2) / logits_teacher.shape[0]
    # loss = -(teacher_prob * pred_prob).sum() / logits_teacher.shape[0]
    return loss
...
def parse_args():
    ...
    parser.add_argument(
        "--task_name",
        default='hate,stance,stance/face_masks_sp,stance/fauci_sp,stance/school_closures_sp,stance/stay_at_home_orders_sp,hate,sem-17,sem-18,wtwt,wtwt/CVS_AET,wtwt/CI_ESRX,wtwt/ANTM_CI,wtwt/AET_HUM,wtwt/FOXA_DIS',
        type=str,
        required=False,
    ...
    parser.add_argument(
        "--model_teacher",
        default='vinai/bertweet-base',#'./model/',
        type=str,
        required=False,
    )
    ...
    parser.add_argument(
        "--ratio2", default='1', type=str, help="ratio for loss")
    parser.add_argument(
        "--device",
        default="0",
        type=str,
    ...
    for lr in learning_rate:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_data_loader):
                ...
                with torch.no_grad():
                    logits_teacher = model_teacher(input_ids, segment_ids)
                loss = loss_fct(logits, labels.view(-1))
                loss_emoji = teacher_loss(logits_emoji, logits_teacher, T=int(args.ratio2))##############################################
