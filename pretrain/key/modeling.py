...
class RobertaEmbeddings(nn.Layer):
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None):
        if position_ids is None:
            ...
            mask = paddle.cast(input_ids != self.pad_token_id, dtype = paddle.int64)
            incremental_indices = (paddle.cumsum(mask, axis=-1) ) * mask
            position_ids = incremental_indices + self.pad_token_id
        ...
        # print(paddle.mean(input_embedings), paddle.mean(position_embeddings), paddle.mean(token_type_embeddings))
