...
def evaluate(model, loss_fct, data_loader):
    ...
    pr_macro =  rep['macro avg']['precision']
    re_macro =  rep['macro avg']['recall']
    print("eval loss: %.5f, f1:%.5f, precision: %.5f, recall: %.5f " % (loss.numpy(), f1_macro, pr_macro, re_macro))
    model.train()
    return f1_macro, pr_macro, re_macro
...
def convert_example(example, tokenizer, max_seq_length=128):
        label = example['label']
        example = tokenizer(example['sentence'],truncation=True)#########, max_seq_len=max_seq_length)
...
def do_train(args):
    ...
    print('final')
    print("f1macro:%.5f, precision:%.5f, recall: %.5f, " % (best_metric[0], best_metric[1], best_metric[2]))
    print("f1macro:%.5f, precision:%.5f, recall: %.5f " % (cur_metric[0], cur_metric[1], cur_metric[2]))
    return cur_metric
...
if __name__ == "__main__":
    ...
    for task in ['stance/school_closures', 'stance/stay_at_home_orders']:#['stance/face_masks', 'stance/fauci', 'stance/school_closures', 'stance/stay_at_home_orders']:
        for model_name in [r_dir+'bertweet/']:
            ...
            print('final aveRec:%.5f, f1PN:%.5f, acc: %.5f ' % (sum(ave_metric[:,0])/5, 
                                                                sum(ave_metric[:,1])/5, 
                                                                sum(ave_metric[:,2])/5) )
            with open('results_stance_continue_bertweet.txt', 'a') as f_res:
                f_res.write(model_name + '\n')
                f_res.write('Task: %s, F1:%.5f, precision:%.5f, recall: %.5f \n' % (task, sum(ave_metric[:,0])/5, 
                                                                            sum(ave_metric[:,1])/5, 
                                                                            sum(ave_metric[:,2])/5) )
                f_res.close()