...
def evaluate(model, loss_fct, data_loader):
    ...
    avePre =  rep['macro avg']['precision']
    aveF1 =  rep['macro avg']['f1-score']
    
    print("eval loss: %.5f, aveF1: %.5f, aveRec:%.5f, avePre:%.5f " % (loss.numpy(), aveF1, aveRec, avePre))
    model.train()
    return aveF1, aveRec, avePre
...
def do_train(args):
    ...
    for TIME in range(10):
        ...
        label2idx = read_label(args.input_dir + 'train_' + str(TIME))
        train_ds = load_dataset(read_data, fileName=args.input_dir + 'train_' + str(TIME), label2idx=label2idx, lazy=False)
        trans_func = partial(
            convert_example,
            tokenizer=tokenizer,
        ...
        dev_ds = load_dataset(read_data, fileName=args.input_dir + 'dev_' + str(TIME), label2idx=label2idx, lazy=False)
        ...
        test_ds = load_dataset(read_data, fileName=args.input_dir + 'test_' + str(TIME), label2idx=label2idx, lazy=False)
        ...
        # print("aveF1: %.5f, aveRec:%.5f, avePre:%.5f " % (best_metric[0], best_metric[1], best_metric[2]))
        # print("aveF1: %.5f, aveRec:%.5f, avePre:%.5f " % (cur_metric[0], cur_metric[1], cur_metric[2]))
        metric_time.append(cur_metric)
    metric_time = np.array(metric_time)
    metric_time = np.mean(metric_time, axis=0)
    print("############FINAL###########aveF1: %.5f, aveRec:%.5f, avePre:%.5f " % (metric_time[0], metric_time[1], metric_time[2]))
    return metric_time
...
if __name__ == "__main__":
    ...
    #     for model_name in [r_dir+'600000/', r_dir+'720000/', r_dir+'840000/', r_dir+'960000/', r_dir+'1080000/', r_dir+'1200000/', r_dir+'1320000/']:
        ...
        for model_name in [r_dir+'200000/', r_dir+'600000/', r_dir+'2200000/', r_dir+'2400000/']:
            ...
            print('aveF1: %.5f, aveRec:%.5f, avePre:%.5f ' % (sum(ave_metric[:,0])/5, 
                                                                sum(ave_metric[:,1])/5, 
                                                                sum(ave_metric[:,2])/5) )
            with open('results_bertweet_complaint.txt', 'a') as f_res:
                f_res.write(model_name + '\n')
                f_res.write('Task: %s, aveF1: %.5f, aveRec:%.5f, avePre:%.5f \n' % (task, sum(ave_metric[:,0])/5, 
                                                                            sum(ave_metric[:,1])/5, 
                                                                            sum(ave_metric[:,2])/5) )
                f_res.close()